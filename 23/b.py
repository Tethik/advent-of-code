...
def in_dest_room(t, y, x):
    return y in [2, 3, 4, 5] and dest_room_x.get(t) == x
...
class State:
    ...
    def next(self, height, width):
        ...
        for i, a in enumerate(self.amphis):
            ...
            # print(t, s, y, x)
            if in_dest_room(t, y, x):
                # print("in dest", (5, x), self.get(5, x))
                # print(t, y, x, 3, x, self.get(3, x))
                if all(self.get(y1, x) == t for y1 in range(5, y, -1)):  # avoid blocking in an amphipod
                    continue
            ...
            if in_hallway(y, x):
                ...
                for dest, t2 in p.items():
                    ...
                    if in_dest_room(t, dest[0], dest[1]):
                        ...
                        if any(self.get(y1, dest[1]) != t for y1 in range(5, dest[0], -1)):
                            continue
                        am = self.amphis.copy()
